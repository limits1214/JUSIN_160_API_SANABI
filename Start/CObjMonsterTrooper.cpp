...
int CObjMonsterTrooper::Update()
{
	...
	if (m_eAniState == AST_PODPOPUP_START
		|| m_eAniState == AST_PODPOPUP_ING
		|| m_eAniState == AST_PODPOPUP_END
		||
	...
	{
		CTimeMgr::Delay(&m_dwFireRateInterval, 100,
			[&]()
			{
				if (m_pTarget != nullptr)
				{
...
void CObjMonsterTrooper::On_Collision(CObj* pObj, COLLISIONID eCollID, void* etc)
{
...
void CObjMonsterTrooper::DeltaUpdate()
{
...
void CObjMonsterTrooper::Apply_Gravity()
{
	if (m_bGravity)
	{
...
void CObjMonsterTrooper::Move(float fAngle, float fLength)
{
